...
# first argument is the name of the training file
cat $1
SIZE=$2
SAMPLE=1e-3
WINDOW=10

awk 'BEGIN{a=0;}{print "_*" a " " $0; a++;}' < $1 > alldata-id.txt
gshuf alldata-id.txt > alldata-id-shuf.txt

time ./word2vec -train alldata-id-shuf.txt -output vectors.txt -cbow 0 -size $SIZE -window $WINDOW -negative 5 -hs 1 -sample $SAMPLE -threads 40 -binary 0 -iter 20 -min-count 1 -sentence-vectors 1
